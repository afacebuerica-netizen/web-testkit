...
import { getFilenameFromUrl, writeJsonFile } from '../utils/file-utils';
  ...
  for (const testUrl of TEST_URLS) {
    test(`audit: ${testUrl}`, async ({ page }) => {
      ...
      try {
        ...
        const filename = getFilenameFromUrl(currentUrl, 'json');
        ...
        const filename = getFilenameFromUrl(testUrl, 'json');
        const filePath = path.join(REPORTS_DIR, `error-${filename}`);
